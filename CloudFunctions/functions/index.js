        ...
        .region("europe-west1").https.onRequest((request, response) => {
          ...
          admin.database().ref("/Posts/")
              ...
              .then((results) => {
                ...
                posts.sort((a, b) => (
                new Date(a.dateStamp) <
                new Date(b.dateStamp) ?1 : -1));
                response.send(posts);
              });
        });

exports
    .eventAdded = functions
        .region("europe-west1").database.ref("/Events/{event_id}")
...
exports.alertAdded = functions
    .region("europe-west1").database.ref("/Alerts/{alert_id}")
    .onCreate((snapshot, context) => {
      ...
      admin.database().ref("/Hospitals/"+alertData.owner)
          .once("value", (snap) => {
            ...
            switch (topic) {
              case "Apos":
                messageCondition="'Apos' in topics || " +
                  "'Aneg' in topics || " +
                  "'Oneg' in topics || 'Opos' in topics";
                break;
              case "Opos":
              ...
              case "Bneg":
                ...
                break;
              case "ABneg":
                messageCondition = "\"ABneg\" in topics || " +
                  "\"Aneg\" in topics || " +
                  "\"Oneg\" in topics || " +
                  "\"Bneg\" in topics";
            ...
            console
                .log("The read failed: " + errorObject.name);
          });
    });
let date;
let eventTime;
...
exports
    .scheduledEventDateCheck = functions
        .region("europe-west1").pubsub.schedule("30 22 * * *")
          ...
          admin.database().ref("/Events")
              ...
              .then((results) => {
                results.forEach((snapshot) => {
                  ...
                  eventTime = (new Date(date+" 20:00:00"))
                      .getTime(); // 23:55 PM - GMT
                  if (eventTime<currentTime) {
                    console.log(currentTime);
                    const delRef = admin.database()
                        .ref("/Events/" + snapshot.key);
                    delRef.remove()
                        .then(() => {
...
exports
    .deleteAlerts = functions
        .region("europe-west1").pubsub.schedule("59 23 * * *")
        .timeZone("Europe/Athens")
        .onRun((context) => {
